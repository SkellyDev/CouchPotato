...
Compared with the previous project delivery, we made lots of changes for the NLP processing part. Before this version, we wrote own trained data by listing sample questions for each scenario(environment describing function) and built a Deep Natural Network Model to match user command with corresponding scenario. Then, we used nltk.tagger tool to extract Noun words from user command and pass them as argument based on the parameter of functions. However, some limitations exist: 1/ our accuracy of DNN model result was highly depend on our training data. 2/our previous training data cannot handle nested function calling situation. If we want to handle it, we need to create much more training data. 3/Pass argument only based on Noun tags will lead to information inaccuracy. The main challenge is that, instead of matching our functions with specific language description/user questions, we want our NLP processing and functions to deal with languages with greater scope and flexibility when asking questions. In other words, our chatbot needs to have a clear understanding on the individual parts of the speech and the dependencies in between; in order to connect user questions with correct environmental describing function and pass corresponding arguments. For instance, consider two questions:

- "How many sheep are inside the house"
- "What animals are inside the house"

The meanings of these sentences are quite different, and our chatbot need to understand the first question is asking about "count(cows, inside(house))", and the second question is asking about "find(animals, inside(house))". In order to achieve this, it is necessary for us to understand the syntatic structure of user's question, so we can have more flexibility. We utilized AllenNLP constituency parsing tools into our project in order to get a syntatic tree model, and recursively visit the tree by finding nodes that will used as function matching or argument passing. In this case, we could avoid the limitation that our DNN model result exclusively on the training data. 
...
a) In order to evaluate the TreeNode class, we built a **iter** function in the class, in order to visually evaluate if it succesfully match syntatic label with its covering text. Since the success of our class TreeNode is discrete, by printing out each node's label and text, we are able to manually compare it with the constructed constituency tree and tell if it is successful or not. The TreeNode class got 100% accuracy during the test process. 

b) For the qualitative environment describing function testing, we manually compare the terminal output with what we truly perceive in the game world. There are some failure tests happened during test process, and most of cases have been solved. Some failure cases still exist when we did the final testing, so we wrote them as functions' limitation. For instance, the getDirection will give us incorrect output when the face has different direction degree as body. Figure 10 shows six passed scenario examples.

<p><img src="assets/execution_scenario.png" width="900" alt/><em>Figure 10: Real execution scenario</em></p>


c) We tested the TreeVisitor class after testing TreeNode and function. Since the TreeVisitor class used the return value of TreeNode as input and connect user command with our environmental describing functions, we need to make sure the accuracy of the first two phrases before going to this step. In this phrase, we focus on evaluating if 1/it successfully extract information to connect the input (user question) with our function, and 2/if the argument is positioned into the right place. We tested TreeVisitor class by connecting with our environment describing functions in order to visually see the pass/failture of our class in Malmo. There are some failure cases exist when testing TreeVisitor class. For instance, we didn't handle the situation of declarative sentence question for some questions, which will cause wrong connect between user question and functions. 
